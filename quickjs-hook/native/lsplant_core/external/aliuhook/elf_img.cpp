        ...
        switch (section_h->sh_type) {
            case SHT_DYNSYM:
                if (bias == -4396) {
                    ...
                    dynsym_start = reinterpret_cast<Elf_Sym *>(((uintptr_t) hdr) +
                                                               dynsym_offset);
                    LOGD("dynsym header 0x%llx size %llu", static_cast<unsigned long long>(section_h->sh_offset), static_cast<unsigned long long>(section_h->sh_size));
                }
                break;
            case SHT_SYMTAB:
                if (strcmp(sname, ".symtab") == 0) {
                    ...
                    symtab_start = reinterpret_cast<Elf_Sym *>(((uintptr_t) hdr) +
                                                               symtab_offset);
                    LOGD("symtab header 0x%llx size %llu found in %s", static_cast<unsigned long long>(section_h->sh_offset), static_cast<unsigned long long>(section_h->sh_size), debugdata_offset != 0 ? "gnu_debugdata" : "orgin elf");
                }
                break;
            case SHT_STRTAB:
                if (bias == -4396) {
                    ...
                    strtab_start = reinterpret_cast<Elf_Sym *>(((uintptr_t) hdr) +
                                                               symstr_offset);
                    LOGD("strtab header 0x%llx size %llu", static_cast<unsigned long long>(section_h->sh_offset), static_cast<unsigned long long>(section_h->sh_size));
                }
            ...
            case SHT_PROGBITS:
                if (strcmp(sname, ".gnu_debugdata") == 0) {
                    ...
                    LOGD("gnu_debugdata header 0x%llx size %llu", static_cast<unsigned long long>(section_h->sh_offset), static_cast<unsigned long long>(section_h->sh_size));
                }
