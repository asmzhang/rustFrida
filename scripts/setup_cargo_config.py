...
def detect_ndk_root(cli_value: str | None) -> Path:
    ...
    for env_name in ("ANDROID_NDK_ROOT", "ANDROID_NDK_HOME"):
        ...
        if env_value:
            ...
            if env_path.parent.name == "ndk":
                versions = sorted((path for path in env_path.parent.iterdir() if path.is_dir()), reverse=True)
                ndk_25 = [v for v in versions if "25." in v.name]
                candidates.extend(ndk_25)
    ...
    if android_home:
        ...
        if ndk_dir.is_dir():
            ...
            # Prioritize NDK 25 to avoid Clang 17 strict compilation errors
            ndk_25 = [v for v in versions if "25." in v.name]
            others = [v for v in versions if "25." not in v.name]
            candidates.extend(ndk_25 + others)
...
def render_config(ndk_root: Path, host_tag: str, api_level: int) -> str:
    ...
    if sys.platform == "win32":
        aarch64_linker_str = f"linker = {toml_string(str(bin_dir / 'clang.exe'))}"
        x86_64_linker_str = f"linker = {toml_string(str(bin_dir / 'clang.exe'))}"
        aarch64_extra_flags = f'rustflags = ["-C", "link-arg=-target", "-C", "link-arg=aarch64-linux-android{api_level}"]'
        x86_64_extra_flags = f'rustflags = ["-C", "link-arg=-target", "-C", "link-arg=x86_64-linux-android{api_level}"]'
    else:
        aarch64_linker_str = f"linker = {toml_string(str(aarch64_clang))}"
        x86_64_linker_str = f"linker = {toml_string(str(x86_64_clang))}"
        aarch64_extra_flags = ""
        x86_64_extra_flags = ""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=33,
        help="Android API level embedded in clang wrapper names. Default: 33.",
    )
    parser.add_argument(
        "--output",
        type=Path,
