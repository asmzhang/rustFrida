...
[[maybe_unused]] bool HookRaw(void *target_art_method, void *hook_art_method, void *backup_art_method) {
    auto *target = reinterpret_cast<ArtMethod *>(target_art_method);
    auto *hook = reinterpret_cast<ArtMethod *>(hook_art_method);
    auto *backup = reinterpret_cast<ArtMethod *>(backup_art_method);
    if (!target || !hook || !backup) {
        LOGE("HookRaw received null ArtMethod");
        return false;
    }
    if (IsHooked(target, true)) {
        LOGW("HookRaw skip duplicate hook");
        return false;
    }
    if (!DoHook(target, hook, backup)) {
        return false;
    }
    RecordHooked(target, target->GetDeclaringClass()->GetClassDef(), nullptr, backup);
    RecordJitMovement(target, backup);
    RecordDeoptimized(hook->GetDeclaringClass()->GetClassDef(), backup);
    return true;
}

[[maybe_unused]] bool UnHookRaw(void *target_art_method) {
    auto *target = reinterpret_cast<ArtMethod *>(target_art_method);
    if (!target) {
        LOGE("UnHookRaw target is null");
        return false;
    }
    art::ArtMethod *backup = nullptr;
    if (!hooked_methods_.erase_if(target, [&backup](const auto &it) {
            backup = it.second.second;
            return backup != nullptr;
        })) {
        LOGE("UnHookRaw target is not hooked");
        return false;
    }
    hooked_methods_.erase(backup);
    hooked_classes_.erase_if(target->GetDeclaringClass()->GetClassDef(), [&target](auto &it) {
...
[[maybe_unused]] bool IsHookedRaw(void *art_method) {
    auto *method = reinterpret_cast<ArtMethod *>(art_method);
    if (!method) {
        return false;
    }
    return IsHooked(method);
}

[[maybe_unused]] bool DeoptimizeRaw(void *art_method) {
    auto *method = reinterpret_cast<ArtMethod *>(art_method);
    if (!method) {
        LOGE("DeoptimizeRaw method is null");
        return false;
    }
    RecordDeoptimized(method->GetDeclaringClass()->GetClassDef(), method);
    if (auto *backup = IsHooked(method); backup) {
        method = backup;
    }
    if (!method || method->IsNative()) {
        return false;
    }
    return ClassLinker::SetEntryPointsToInterpreter(method);
}
